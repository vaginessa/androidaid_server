...
$app_channel = '3gstone';
#WHERE 
$where = "app_state = '$app_state_deploy' AND app_channel = $app_channel";
$order = DB_COLUMN_APP_CATEGORY." ASC";
...
#	$app_url=mysql_result($result,$i,DB_COLUMN_APP_URL);
	...
	$array = Array(DB_COLUMN_APP_CATEGORY=>$app_category,DB_COLUMN_APP_PACKAGE=>$app_package, DB_COLUMN_APP_LABEL=>$app_label,DB_COLUMN_APP_VERSION_CODE=>$app_version_code,
		DB_COLUMN_APP_FILE=>$app_file,DB_COLUMN_APP_STATE=>$app_state,DB_COLUMN_APP_PRICE=>$app_price,DB_COLUMN_APP_UPDATE_STAMP=>$app_update_stamp);
...
date_default_timezone_set('Asia/Shanghai'); //สฑฒ๎8ะกสฑ
$app_update_stamp =  date("Y-m-d H:i:s");

$api_version = 1;

$array = Array(API_JSON_RESPONSE=>$api_response,API_VERSION=>$api_version,DB_COLUMN_APP_UPDATE_STAMP=>$app_update_stamp,API_JSON_RESPONSE_NUM=>$num,API_JSON_APP_ARRAY=>$jsonobj);
echo json_encode($array);

file_put_contents("app_repo.json",json_encode($array));
